...
with db:
    ...
    fig1 = px.bar(category_df, x="category", y="total", text=['${:,.2f}'.format(x) for x in category_df["total"]],template="seaborn")
...
with db:
    ...
    fig3 = px.bar(filtered_df.sort_values("total", ascending=False).head(10), x="full_name", y="total", template="seaborn")
    st.plotly_chart(fig3, height=200)

# Un histogramme qui donne la répartition de l’âge des clients :
with dc:
    fig4 = px.histogram(filtered_df, x="age", title="Répartition de l'âge des clients", template="seaborn")
...
# data_by_state.rename(columns={"State Complet": "locations"})
# # Création de la carte
# fig7 = px.choropleth(data_by_state, locations="locations", color="total", hover_data=["total"], color_continuous_scale="YlOrRd")
